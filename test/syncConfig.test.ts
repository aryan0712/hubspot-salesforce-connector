...
describe('sync configuration', () => {
  it('defaults every canonical object to bidirectional sync', () => {
    const config = defaultSyncConfig('last-write-wins', 'salesforce');
  ...
  it('enforces object pauses and one-way directions', () => {
    const config = defaultSyncConfig('field-merge', 'hubspot');
  ...
  it('returns isolated configuration snapshots', async () => {
    const initial = defaultSyncConfig('last-write-wins', 'salesforce');
